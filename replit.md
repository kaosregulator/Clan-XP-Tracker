...
- **Submission model**: `clans.autoApprove` (default true) makes submissions count immediately (no review queue); set it false in `/setup` (Identity modal) for staff review. Submit is a notes modal + optional screenshot follow-up (Discord modals can't hold images). Members can hit **Vacation** (a logged negative mark, not an excuse). The role-scoped admin **tracker** embed (`features/tracker.ts`) lives in `trackerChannelId`, shows completed/missing/vacation vs. the `requiredRoleId` members, and auto-updates.
