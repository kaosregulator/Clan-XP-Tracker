...
- **Schema changes require a DB push**: after pulling, run `pnpm --filter @workspace/db run push` to create the new columns/tables (`tracked_accounts`, `reminders`, `dashboards`, `vacations`, plus new `clans` columns incl. `auto_approve`/`required_role_id`/`tracker_channel_id`/`clan_rank`, `clan_members.vacation_count`, `xp_submissions.extracted`). All additions are additive.
- **Live dashboards are embeds that edit one message per (guild, type).** `features/dashboard.ts` builds three embeds — **Clan** (public: total/today/week XP, goal + capacity bars, active members, top contributors, clan rank if set), **Alt** (`patriot` type: combined/today alt XP, alt share of clan XP, top alt contributors), **Admin** (`staff` type: all of the above + pending, verification queue, late/missed, warnings, inactivity, Patriot/alt counts, recent activity log). They edit in place via the `dashboards` table (`upsertDashboardMessage` sets `attachments: []` so a legacy image board flips to an embed). XP is derived from the contribution model (`contributions`/`altAccountsCompleted` × `contributionValue`) in `services/dashboardStats.ts` — the legacy `xp_*` columns stay zero. Auto-refresh is debounced (`scheduleDashboardRefresh`, 4s) and fires on every XP change (submit, approve/reject, alt submit, vacation); the scheduler still refreshes every 5 min as a backstop. The **Leaderboard** board is a separate canvas card and is intentionally left as-is.
...
- **Hubs are embeds, not canvas.** `/xp` (member hub), `/xpadmin` (staff hub) and `/leaderboard` render as Discord embeds — no PNG upload, no off-thread render, no avatar download (the avatar is an embed thumbnail URL). This is the big latency win: file attachments cost ~1–3 s each on every interaction/refresh, embeds are inline JSON and near-instant. `/profile` reuses the member-hub embed. `/report`, `/help`, the tracker board and dashboards still render canvas cards (periodic/less latency-sensitive) — migrate them the same way if needed.
