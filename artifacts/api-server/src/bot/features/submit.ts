...
function submitModal(clan: Clan, fromGuildId?: string): ModalBuilder {
  ...
  const customId = fromGuildId
    ? submitModalForGuild(fromGuildId)
    : XP_SUBMIT_MODAL;
...
async function finalize(
  ...
  if (auto) {
    ...
    scheduleDashboardRefresh(client, clan);
  } else {
...
async function runExtractionIfEnabled(
  clan: Clan,
  sub: XpSubmission,
...
export async function handleSubmitModal(interaction: ModalSubmitInteraction) {
  ...
  const guildId =
    parseId(interaction.customId).arg ?? interaction.guildId ?? undefined;
  if (!guildId) return;
  ...
  const displayName = interaction.inCachedGuild()
    ? interaction.member.displayName
    : undefined;
  const identity = identityFromUser(interaction.user, displayName);
  await ensureMember(clan.guildId, identity);
  const notes =
    interaction.fields.getTextInputValue("notes")?.trim().slice(0, 1000) ||
    null;
  const alts = clan.altAccountsEnabled
    ? parseAlts(interaction.fields.getTextInputValue("alts"))
    : 0;
  ...
  await interaction.editReply(
    await confirmText(clan, contributions, overflow, auto),
  );
}
...
async function confirmText(
  ...
  const altNote =
    contributions > 1
      ? ` (you + ${contributions - 1} alt${contributions - 1 === 1 ? "" : "s"})`
      : "";
...
export async function handleSubmissionMessage(message: Message): Promise<void> {
  ...
  try {
    const identity: MemberIdentity = identityFromUser(
      message.author,
    ...
    const altPending = await latestPendingAwaitingProof(
      clan.guildId,
      message.author.id,
    );
    ...
    const awaiting =
      altPending &&
      Date.now() - altPending.submittedAt.getTime() <= PROOF_LINK_WINDOW_MS
        ? altPending
    ...
    if (
      awaiting &&
      Date.now() - awaiting.submittedAt.getTime() <= PROOF_LINK_WINDOW_MS
    ) {
    ...
    if (brandNew) {
      ...
      scheduleDashboardRefresh(message.client, clan);
    }
